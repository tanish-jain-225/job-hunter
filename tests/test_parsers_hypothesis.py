...
_lever_job = st.fixed_dictionaries({
    "id": _text,
    "text": _text,
    "categories": st.none() | st.fixed_dictionaries({
        "location": _opt_text,
        "team": _opt_text,
    }),
...
@given(
    ...
    body=st.fixed_dictionaries({
        "jobs": st.lists(_greenhouse_job, max_size=10)
    }) | st.just({}) | st.just({"jobs": None}) | st.just(None),
)
...
@given(
    slug=st.text(min_size=1, max_size=50, alphabet=st.characters(whitelist_categories=("L",))),
    body=st.fixed_dictionaries({
        "jobPostings": st.lists(_ashby_job, max_size=10)
    }) | st.just({}) | st.just(None),
)
@settings(max_examples=200, suppress_health_check=[HealthCheck.too_slow])
...
@given(
    slug=st.text(min_size=1, max_size=50, alphabet=st.characters(whitelist_categories=("L",))),
    body=st.fixed_dictionaries({
        "jobs": st.lists(_workable_job, max_size=10)
    }) | st.just({}) | st.just(None),
)
@settings(max_examples=200, suppress_health_check=[HealthCheck.too_slow])
    ...
    body=st.fixed_dictionaries({
        "content": st.lists(
            st.fixed_dictionaries({
                ...
                "jobAd": st.none() | st.fixed_dictionaries({
                    "sections": st.none() | st.fixed_dictionaries({"jobDescription": st.none() | st.fixed_dictionaries({"text": _opt_text})})
                }),
                "applyUrl": _opt_text,
...
def test_job_score_100_clamped() -> None:
    ...
    for score in [-99.0, 0.0, 5.5, 10.0, 10.1, 999.0, None]:
        job = Job(
            job_id="test:co:1", ats="test", company="Co", title="SWE",
            location="India", url="#", description="x", score=score,
        )
...
def test_job_to_dict_has_required_keys() -> None:
    ...
    job = Job(job_id="a:b:c", ats="a", company="B", title="T",
              location="L", url="U", description="D", score=8.5)
    d = job.to_dict()
