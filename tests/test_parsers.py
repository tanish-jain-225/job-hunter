...
def test_exclude_locations_drops_out_of_region_remote():
    ...
    kept = prefilter([us_remote, india_remote], dict(FILTERS, exclude_locations=[r"\b(united states|usa)\b"]))
    assert len(kept) == 1
            ...
            "positions": [
                {
                    ...
                    "location": {"name": "Berlin", "is_remote": True},
...
def test_detect_job_type_and_negations():
    ...
    j_not_remote = Job(
        "5", "gh", "A", "DevOps", "Delhi", "http://x", "Candidate must work in office. Not remote / no wfh."
    )
    assert "remote" not in _detect_job_type(j_not_remote)

    # Negation rules: 'not an internship'
    j_not_intern = Job(
        "6", "gh", "A", "Engineering Manager", "Bengaluru", "http://x", "Leadership role (not an internship)."
    )
