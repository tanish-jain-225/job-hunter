...
- **Screen** — batch ~8–15 jobs per call, truncate each JD to ~1800 chars, return JSON array of `{job_id, score, reason}`. Handled by **Groq** (`llama-3.1-8b-instant`, 14,400 RPD free) for high throughput.
