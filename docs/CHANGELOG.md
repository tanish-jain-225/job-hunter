...
### Maintenance
- **Expanded Test Suite to 424 Passed Tests**: Added test coverage for notice period and CTC preferences (`test_profile_preferences_notice_and_ctc` in `tests/test_resume_studio.py`), referral request drafting (`test_draft_includes_referral_request_and_context` in `tests/test_llm.py`), and 94-company YAML integrity (`test_companies_yaml_integrity` in `tests/test_clean_and_verify.py`), bringing the test suite to **424 passed tests** with **90%+** code coverage.
