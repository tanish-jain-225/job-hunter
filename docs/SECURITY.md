...
- **Offline JWT Signature Verification & Fallback**: Validates Supabase JWT access tokens offline using PyJWT and `SUPABASE_JWT_SECRET` (HMAC-SHA256) with strict issuer (`iss`) and audience (`aud`) claims validation, providing sub-millisecond local verification with zero external network hops and complete immunity to third-party API rate limits on serverless environments. If `SUPABASE_JWT_SECRET` is not set, it securely falls back to validating with Supabase Auth API (`GET /auth/v1/user`) with an in-memory cache.
- **Resilient Client-Side Session Lifecycle**: Frontend `authFetch` ([`static/js/app.js`](../static/js/app.js)) inspects token lifetime (`expires_at`), proactively refreshing nearing tokens (<90s remaining) before request dispatch, and silently retries on any transient 401s before session invalidation, avoiding session drops due to background tab timer throttling or wake-from-sleep.
